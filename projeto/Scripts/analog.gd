...
signal force_change(inForce, inStick)
...
#	halfSize.x *= scale.x;
#	halfSize.y *= scale.y;
	squaredHalfSizeLength = halfSize.x * scale.x * halfSize.y * scale.y;
...
func calculateForce(var x, var y):
	#get direction
	currentForce.x = (x - centerPoint.x)/(halfSize.x * scale.x);
	currentForce.y = -(y - centerPoint.y)/(halfSize.y * scale.y);
...
func sendSignal2Listener():
	...
	emit_signal("force_change",currentForce, self)
